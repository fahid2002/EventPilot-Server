...
export async function listEvents(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const sortOption = String(req.query.sort || "date");
    const sort: any = sortOption === "price" ? { price: 1 } : sortOption === "rating" ? { rating: -1 } : { date: 1 };
    ...
    res.json({ success: true, message: "Events loaded.", data: { events, total, page, pages: Math.ceil(total / limit) || 1 } });
  } catch (error) {
    next(error);
  }
}
...
export async function getEvent(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    res.json({ success: true, message: "Event loaded.", data: { event } });
  } catch (error) {
    next(error);
  }
}
...
export async function createEvent(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    res.status(201).json({ success: true, message: "Event submitted for admin approval.", data: { event } });
  } catch (error) {
    next(error);
  }
}
...
export async function deleteEvent(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    res.json({ success: true, message: "Event deleted.", data: { deleted: true } });
  } catch (error) {
    next(error);
  }
}
